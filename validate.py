...
def recursively_read(rootdir, must_contain, exts=["png", "jpg", "JPEG", "jpeg", "bmp"]):
    out = [] 
    print(f"\n[Debug] Recursively reading from: {rootdir}")
    print(f"[Debug] Allowed extensions: {exts}")
    for r, d, f in os.walk(rootdir):
        print(f"[Debug] Current directory: {r}")
        print(f"[Debug] Files found here: {f[:5]}")  # 打印前5个文件，避免过长
        for file in f:
            ...
            if ext in exts and must_contain in full_path:
                ...
                # 打印为什么文件被跳过
                if ext not in exts:
                    print(f"[Debug] Skipped {full_path} (invalid extension: {ext})")
                elif must_contain not in full_path:
                    print(f"[Debug] Skipped {full_path} (missing substring: '{must_contain}')")
    return out
...
def get_list(path, must_contain=''):
    ...
    print(f"[Debug] Must contain substring: '{must_contain}'")
    print(f"[Debug] Found {len(image_list)} images")
    if len(image_list) > 0:
        print("[Debug] Example image paths:")
        for p in image_list[:3]:  # 打印前3个文件的路径
            print("   ", p)
    return image_list
...
class RealFakeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"\n[Error] 无法加载图像 {img_path}: {str(e)}")
        # 返回空数据或跳过（此处示例返回随机数据）
            return torch.zeros(3, 224, 224), label  # 根据你的模型输入调整
...
if __name__ == '__main__':
    ...
    if args.real_path is not None and args.fake_path is not None:
    # 生成一个临时配置项
        custom_dataset = {
            "real_path": args.real_path,
            "fake_path": args.fake_path,
            "data_mode": "wang2020",  # 可自定义其他字段
            "key": "cunstom"   # 标识为用户自定义路径
        }
        DATASET_PATHS = [custom_dataset]  # 覆盖原有的硬编码配置
    else:
    # 如果没有提供命令行参数，使用默认配置（但此时 DATASET_PATHS 已被清空）
        pass
