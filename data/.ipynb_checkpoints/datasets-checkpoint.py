...
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        if opt.data_mode == 'ours':
            ...
            # 根据训练或验证阶段选择父目录
            parent_dir = 'train' if opt.data_label == 'train' else 'test'
            # 真实图像路径：datasets/train/progan/0_real
            real_path = os.path.join(opt.wang2020_data_path, parent_dir, 'progan', '0_real')
            # 伪造图像路径：datasets/train/progan/1_fake
            fake_path = os.path.join(opt.wang2020_data_path, parent_dir, 'progan', '1_fake')
            # 加载路径
            real_list = get_list(real_path, must_contain='')  # 无需 must_contain 过滤
        ...
        print(f"[Debug] Wang2020 真实图像路径: {real_path}")
        print(f"[Debug] Wang2020 真实图像数量: {len(real_list)}")
        print(f"[Debug] Wang2020 伪造图像路径: {fake_path}")
        print(f"[Debug] Wang2020 伪造图像数量: {len(fake_list)}")
